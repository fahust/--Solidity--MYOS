...
contract ProxyEquipments is Ownable, ReentrancyGuard {
  ...
  function buyEquipment(
    ...
    EquipmentsLib.Equipment memory equipment = equipmentContrat.getEquipmentDetails(tokenId);
  ...
  function sellEquipment(uint256 quantity, uint256 tokenId) external {
    ...
    if (sent == false)
      revert SellEquipmentSendEth({ to: _msgSender(), value: equipment.price * quantity });
  ...
  function convertToAnotherToken(
    ...
    Equipments equipmentContrat = Equipments(addressEquipment);
    EquipmentsLib.Equipment memory fromEquipment = equipmentContrat.getEquipmentDetails(fromTokenId);
    EquipmentsLib.Equipment memory toEquipment = equipmentContrat.getEquipmentDetails(toTokenId);
