...
contract DelegateContract is Ownable {
    ...
    */
     ...
     */
    function setParamsDelegate(string memory keyParams, uint256 valueParams)
        internal
    {
        Hero contrat = Hero(addressContract);
     ...
     */
    function buyItem(address payable addressItem, uint256 value)
        public
        payable
    {
        (bool sent, bytes memory data) = addressItem.call{value: msg.value}(
            abi.encodeWithSignature(
                "buyItem(uint256,address)",
                value,
                msg.sender
            )
        );
    ...
    function getCurrentPrice(address addressItem)
        public
        view
        returns (uint256)
    {
     ...
     */
    function randomParams8(uint8 peuple)
        internal
        virtual
    ...
    function completeQuest(uint256 tokenId) public {
        ...
        Quest questContrat = Quest(addressQuest);
        Quest.Quest memory questTemp = questContrat.getQuestDetails(
    ...
    function getQuest(uint8 questId)
        external
        view
        returns (Quest.Quest memory)
    {
