...
contract DelegateContractMYOS is Ownable {
    ...
    function setMerkleTree(bytes32 _merkleRoot, uint256 _merkleEndTime)
        external
        onlyOwner
    {
    ...
    function buyMYOS(
        ...
        if (currentpriceMYOS != 0)
            require(
                msg.value >= (currentpriceMYOS * quantity),
                "More ETH required"
            );
        if (currentpriceMYOS == 0)
            require(
    ...
    function sellMYOS(uint256 quantity) public {
        require(
            MYOS(addressMYOSToken).totalSupply() > quantity + 1,
            "No more this token"
        );
        require(
    ...
    function convertMYOSToAnotherToken(uint256 quantity, address anotherToken)
        public
    {
    ...
    function verifyMerkleProof(
        ...
        if (
            ...
            merkleEndTime < block.timestamp &&
            merkleEndTime != 0
        ) {
            (bool validMerkleProof, uint256 merkleProofIndex) = MerkleProofLib
                .verify(
                    _proofs,
                    merkleRoot,
                    keccak256(
